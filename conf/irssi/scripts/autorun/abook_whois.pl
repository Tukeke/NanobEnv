...
$VERSION = "0.1";
%IRSSI = (
    name	=> "Irssi cpim, integrates irssi with cpim, looking for nickname fields in abook .",
...
sub get_current_nick_data { return `cpim.abook @_ `;}
sub get_current_nick_mobile { return `cpim.abook @_|cut -d, -f2|cut -d" " -f2 &`; }
sub call{ system("cpim.call", "+34$_[0]");return;}
sub kill_skype{system('cpim.stopcall');}
