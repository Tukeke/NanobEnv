...
Irssi::command_bind('call', 'cmd_call');
sub cmd_call{
    my ($data, $server, $window) = @_;$data=`aboook_parser $data`;if ($data == "") { $data="Noone found"; }
    notify($server, 'CallSystem', $data. ' ', 'Calling...');
}
sub notify {
...
sub print_text_notify {
    ...
    #else{
        #    if ($all[$stripped] < time() + 600){ return } else { $all[$stripped] = time();}
        #}
